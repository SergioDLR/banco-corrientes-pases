...
const exclude = ['codigoTipoOperacion', 'codigoOperacion', 'numerosiopel', 'rueda', 'codigoEntidad', 'nombreEntidad']
const respuesta = {
  metadata: {
    resultset: {
      count: 1,
...
const columnas = Object.keys(respuesta.pases[0].concertacion)
  .map((i: any) => ({ id: i.replace('fecha', ''), label: i.replace('fecha', '') }))
  .filter((i) => !exclude.includes(i?.id))

const filas = [respuesta.pases[0].concertacion, respuesta.pases[0].vencimiento]
